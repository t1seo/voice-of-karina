...
MLX is the chosen local execution route for Apple Silicon. The adapter pins `mlx-audio==0.5.4`; the 1.7B Base revision is `37e955a1deb861c088ae5f3a67043185f3d1a60c`. Model identifiers and revisions are recorded in generated artifacts. The current choice prioritizes quality over the previous 0.6B model's lower generation cost. Generating only requested messages, releasing models between stages, and reusing saved reference audio limit unnecessary work. Legacy requests without explicit recipes still load once per batch. The recipe workflow currently invokes TTS once per message to retain exact attempt and recovery evidence, so multiple messages incur repeated model loading; see the [measured reuse validation and limitations](reusable-voice-validation.ko.md).
